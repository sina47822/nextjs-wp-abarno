...
export function TrendCard({ item }: { item: CardItem }) {
    ...
    return (
        ...
        >
            <Wrapper>
                <div className={`h-44 sm:h-40 ${tone.header}`}></div>
                <div className="p-6 sm:p-7">
                    <h3 className="text-2xl sm:text-3xl font-extrabold leading-snug tracking-tight text-neutral-900">
                        {item.title}
                    </h3>
                    <p className="mt-5 text-sm sm:text-base leading-7 text-neutral-700">{item.description}</p>
                </div>
            </Wrapper>
        </motion.article>
    );
